...
<!-- - `x %>% f %>% g` -- то же, что и `g(f(x))` -->

<!-- ### С нестандартной позицией аргумента -->

<!-- `.` указывает, куда именно справа подставлять левую часть оператора: -->

<!-- - `x %>% f(y, .)` -- то же, что и `f(y, x)` -->
<!-- - `x %>% f(y, z = .)` -- то же, что и `f(y, z = x)` -->

<!-- Больше подробностей: смотреть в [документации `magrittr`](https://magrittr.tidyverse.org/index.html). -->
